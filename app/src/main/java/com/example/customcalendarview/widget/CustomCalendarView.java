...
import com.example.customcalendarview.utils.DateUtils;
...
public class CustomCalendarView extends FrameLayout {
    public static final String TAG = "XXXXXXX";

    protected float FLIP_DISTANCE;

    private SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM");
    private SimpleDateFormat dmyDateFormat = new SimpleDateFormat("yyyy-MM-dd");
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        weekView = view.findViewById(R.id.rv_week);
        ...
        weekView.setLayoutManager(new LinearLayoutManager(context));
...
//                Log.d(TAG, "onBindViewHolder: "+itemVO.getYearMonthDay());
        ...
        mWeekAdapter = new BaseRecycerAdapter<String>(getContext(), mDataTest, R.layout.item_week) {
            @Override
...
//            Log.i(TAG, "initData: "+dmyDateFormat.format(instance.getTime()));

            calendarBean.setYearMonthDay(dmyDateFormat.format(instance.getTime()));

//            Log.i(TAG, "init: "+instance.get(Calendar.YEAR)+"   " + instance.get(Calendar.MONTH) + 1 );
        ...
        //获取当前集合的第一天
//        Log.i(TAG, "initAdapter: "+calendarBeans.get(0).getYearMonthDay());
        //当月第一天
        int weekNo = DateUtils.getWeekNumber(getMonth());
        mDataTest.clear();
        for (int i = 0; i < calendarBeans.size()/7; i++) {
            mDataTest.add(String.valueOf(weekNo));
            weekNo++;
        }
    ...
    private ArrayList<CalendarBean> getLastMonth(CalendarBean calendarBean) {
        ...
        if (firstDayOfWeek != 1) {
            ...
            for (int i = 0; i < needAdd; i++) {
                ...
                dayBean.setYearMonthDay(DateUtils.getPreviousMonth(getMonth())+"-"+lastMonthDay);

//                Log.d(TAG, "golastMonth: "+ DateUtils.getPreviousMonth(getMonth())+"-"+lastMonthDay);
...
//                Log.d(TAG, "goNextMonth: "+ DateUtils.getNextM(getMonth()));
                CalendarBean dayBean = new CalendarBean();
                //0：当月，1：上月，2：下一个月
                dayBean.setDayType(2);
                dayBean.setWeekOfDay(getWeekString(i));
                dayBean.setDate(calendarLastDay.getTime());
                ...
                dayBean.setDay(String.valueOf(lastMonthDay));
                dayBean.setYearMonthDay(DateUtils.getNextM(getMonth())+"-0"+lastMonthDay);

//                Log.e(TAG, "getNextMonth: "+DateUtils.getNextM(getMonth())+"-0"+lastMonthDay);
//                dayBean.setYearMonthDay(DateUtils.getNextM(getMonth()));
